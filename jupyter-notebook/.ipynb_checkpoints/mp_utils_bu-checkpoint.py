...
def sort_dictionary(dictionary, sorting):
    if sorting == None:
        return dictionary
...
class Kernel:
    ...
    def center_frequency(self, fs=16000):
        w, h = sig.freqz(self.kernel, worN = max(len(self.kernel), 2048), fs=fs)  # w in Hz
        ...
        """
        plt.plot(np.pad(self.kernel, (0, 300-len(self.kernel))))
        plt.show()
        plt.plot(w,mag_db)
        plt.show()
        print(center_freq)
