...
def create_dictionary_from_JLD2(filepath, sorting = None, max_pos = False, align_peaks = False):
    """
    ...
    with h5py.File(filepath, "r") as f:
        ...
        for kernel_entry in kernels:
            ...
            if max_pos:
                # Ensure the maximum value is positive
                if np.abs(np.min(kernel_data)) > np.abs(np.max(kernel_data)):
    ...
    if align_peaks:
        ...
        target_idx = max(np.argmax(np.abs(k.kernel)) 
                         for k in dictionary 
                         if k.centroid_frequency <= max_centroid
                        )

        for kernel in dictionary:
            if kernel.centroid_frequency > max_centroid:
                continue  # Skip kernels with centroid frequency above the threshold
            k = kernel.kernel

            peak_idx = np.argmax(np.abs(k))
            shift = target_idx - peak_idx

            if shift > 0:
                # shift right → pad on the left
                k_aligned = np.pad(k, (shift, 0), mode='constant')
            elif shift < 0:
                # shift left → pad on the right
                k_aligned = np.pad(k, (0, -shift), mode='constant')
            else:
                k_aligned = k

            kernel.kernel = k_aligned
    return dictionary
...
def plot_dictionary_elements(dictionary, title = None, show=True, sample_rate = None):
...
def short_time_matching_pursuit_iter(dictionary, x, indx_start, indx_end, stop_type, stop_condition, crop_length):         
    ...
    stop_type : str
        The stopping criterion type for the matching pursuit algorithm (e.g., 'iterations', 'amplitude').
    stop_condition : int or float
        The stopping condition value corresponding to the `stop_type` (e.g., number of iterations or amplitude threshold).
    crop_length : int
...
def plot_encoding(x, sr, encoded_waveform, time_range = None, min_abs_amplitude = 0.001, x_reconstructed = None, show = True, dictionary=None, N_highlights=0, poster=False, spectrogram=False):
    ...
    Parameters:
        x: The original waveform.
        sr: Sampling rate.
        encoded_waveform (list of tuples): Each tuple is (dictionary_element, amplitude, sample_nbr).
        time_range (tuple): (start_time, end_time) in seconds. You can use this to zoom in on the plot.
        min_abs_amplitude (float): Minimum absolute amplitude for an encoding event to be included.
        x_reconstructed: the reconstructed waveform
        show: whether or not to use plt.show()
    ...
    if time_range != None:
        ...
        t_segment = time_axis[s_start:s_end]*1000
    else:
        x_segment = x
        s_start = 0
        s_end = len(x)
        t_segment = time_axis*1000
        t_start = time_axis[0]
        t_end = time_axis[-1]
    ...
    for highlight in highlights:
        if dictionary:
            ...
            sns.lineplot(x=time_ax, y=kernel_ax, ax=axs[0], color="green", alpha=0.6)
        else:
            print("Warning: to show highlights the dictionary should be provided!")
