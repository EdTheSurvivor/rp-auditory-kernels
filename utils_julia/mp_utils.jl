...
module mp_utils
    ...
    function matching_pursuit_iter(x_res, kernels)
        ...
        for (j, (a_j, b_j)) in enumerate(results)
            a[j] = a_j # amplitude
            b[j] = b_j # index
            c[j] = abs(a_j)*kernels[j].weight  # weighted amplitude
        end

        # Find the kernel with the highest absolute value in a
        kernel_val = nothing
        try
        	kernel_val = argmax(abs.(c))
        catch e
        	print("In mp_utils.matching_pursuit_iter...")
        	println(c)
        	println(results)
        	flush(stdout)
        	rethrow()
    ...
    # This function trims and expands the kernels based on a threshold and expansion range
    function trim_and_expand_kernels!(kernels, threshold, expansion_range, orthogonalise_kernels=false, max_length=-1, min_length=-1)
        """
        FunctionName(kernels, threshold, expansion_range)

        Processes a vector of kernel structs based on a specified threshold and expansion range.

        # Arguments
        - `kernels::Vector{T}`: A vector of structs containing the kernels.
        - `threshold::Float64`: The threshold value used to determine whether to expand or reduce.
        - `expansion_range::Int`: The length of the subset of the kernel used to decide whether to expand.

        # Description
        This function operates on a collection of kernel structs, applying expansion or reduction 
        based on the provided threshold and the specified range of the kernel.

        # Returns
        Returns the processed kernels after applying the expansion/reduction logic.

        # Notes
        Ensure that the `kernels` vector contains valid kernel structs and that the `threshold` 
        and `expansion_range` values are appropriately set for the desired operation.
        """
        ...
        for ng in 1:Ng
            ...
            nPad = min(floor(Int, 0.1 * Lg), 6)  # padding length

            if Lg + 2*nPad > max_length && max_length > 0 
                nPad = floor(Int, (max_length - Lg)/2)
            end
            ...
            try
            	...
            	kernels[ng].gradient = vcat(zeros(nPad), kernels[ng].gradient, zeros(nPad))         
    		 	...
    		 	@show e
    			println("In mp_utils.trim_and_expand_kernels!() something goes wrong sometimes. I'm not able to reproduce it consistently. Maybe this catch helps?")
    			println(" The padding length was: ", nPad)
    			println(" The kernel number was: ", ng)
    			println(" The kernel length was: ", length(kernels[ng].kernel))
    			println(" The code will continue now without doing expansion of this kernel this time...")
    		end
        end
        ...
        for ng in 1:Ng
            ...
            while ((LinearAlgebra.norm(kernel[1:exp_indx])^2 < threshold) || (LinearAlgebra.norm(kernel[end-exp_indx+1:end])^2 < threshold)) && (length(kernel) > min_length) 
                if (LinearAlgebra.norm(kernel[1:exp_indx])^2 < threshold) 
                    kernel = kernel[2:end]
                    gradient = gradient[2:end]
                end
                if (LinearAlgebra.norm(kernel[end-exp_indx+1:end])^2 < threshold)
